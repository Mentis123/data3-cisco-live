...
export function createDatabaseStorage(db: NeonDatabase<typeof schema>) {
  ...
  return {
    ...
    try {
      ...
      // If no entries exist (e.g., after reset with no submissions), return empty array
      // The word cloud can be populated by using the "Sync Since Reset" button in admin
      console.log('[getWordCloudData] No active entries in database, returning empty array');
      return [];
    } catch (error) {
